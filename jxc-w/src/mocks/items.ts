import type { ItemListParams, ItemStatus, ItemVO } from '@/api/modules/item';
...
const pad = (value: number, len = 6) => String(value).padStart(len, '0');
...
const createItem = (index: number): ItemVO => {
  const status: ItemStatus = index % 5 === 0 ? '停用' : '启用';
...
let mockItemDb: ItemVO[] = Array.from({ length: 180 }, (_, idx) => createItem(idx + 1));

const toLower = (value: string) => value.toLowerCase();

const filterItems = (items: ItemVO[], params: ItemListParams) => {
...
export const mockFetchItems = async (params: ItemListParams) => {
  await sleep(240);
...
export const mockBatchUpdateStatus = async (ids: string[], status: ItemStatus) => {
  await sleep(180);
  const idSet = new Set(ids);
  mockItemDb = mockItemDb.map((item) => (idSet.has(item.id) ? { ...item, status } : item));
};

export const mockBatchDeleteItems = async (ids: string[]) => {
  await sleep(180);
  const idSet = new Set(ids);
