...
const timeTypeOptions: TimeType[] = ['退料日期', '创建时间'];
...
const tableData: DepartmentReturnRow[] = [
  {
    id: 1,
    documentCode: 'BT-202604-001',
    ...
    reviewStatus: '已复审',
    printStatus: '已打印',
    createdAt: '2026-04-13 11:06:00',
    creator: '张敏',
    remark: '午市多领退回',
  },
  {
    id: 2,
    documentCode: 'BT-202604-002',
    ...
    reviewStatus: '未复审',
    printStatus: '未打印',
    createdAt: '2026-04-12 17:18:00',
    creator: '李娜',
    remark: '晚市退回半成品',
  },
  {
    id: 3,
    documentCode: 'BT-202604-003',
    returnDate: '2026-04-11',
    department: '饮品吧台',
    inboundWarehouse: '南区包材仓',
    amount: '420.00',
    documentStatus: '草稿',
    reviewStatus: '未复审',
    printStatus: '未打印',
    createdAt: '2026-04-11 15:22:00',
    creator: '王磊',
    remark: '未使用包材退回',
  },
];
...
const filteredRows = computed(() => {
  ...
  return tableData.filter((row) => {
    const dateField = query.timeType === '退料日期' ? row.returnDate : row.createdAt.slice(0, 10);
...
const handleSelectionChange = (rows: DepartmentReturnRow[]) => {
  selectedIds.value = rows.map((row) => row.id);
};

const handleView = (row: DepartmentReturnRow) => {
  ElMessage.info(`查看：${row.documentCode}`);
};

const handleEdit = (row: DepartmentReturnRow) => {
...
<template>
  <section class="panel item-main-panel">
    ...
    >
      ...
      <el-table-column prop="returnDate" label="退料日期" min-width="110" show-overflow-tooltip />
      <el-table-column prop="department" label="退料部门" min-width="120" show-overflow-tooltip />
