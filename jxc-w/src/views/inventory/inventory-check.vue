...
const handleExport = async () => {
  ...
  try {
    const exportRows = await fetchAllPages<InventoryCheckRow>(async (pageNum, pageSizeValue) => {
      const page = await fetchInventoryCheckPageApi('inventory-checks', {
        pageNum,
    ...
    const lines = [
      ['单据编号', '盘点日期', '仓库', '物品数', '账面金额', '实盘金额', '盈亏金额', '盘点范围', '盘点频次', '状态', '盘点差异', '审核日期', '打印状态', '创建时间', '创建人', '备注']
        .map(toCsvCell)
        .join(','),
    ];
    exportRows.forEach((row) => {
      lines.push([
        ...
        row.itemCount,
        row.totalBookAmount,
        row.totalActualAmount,
        row.totalDiffAmount,
        checkRangeLabelMap.value[row.checkRangeType] ?? row.checkRangeType,
