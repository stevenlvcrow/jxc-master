...
const documentTimeTypeOptions = ['入库日期', '创建时间'];
const itemTimeTypeOptions = ['移库日期', '创建时间'];
...
const matchesDateRange = (dateRange, businessDate, createdAt, isCreatedTime) => {
...
const documentFilteredRows = computed(() => {
    ...
    return documentRows.filter((row) => {
        const matchedDateRange = matchesDateRange(documentQuery.dateRange, row.inboundDate, row.createdAt.slice(0, 10), documentQuery.timeType === '创建时间');
...
const itemFilteredRows = computed(() => {
    ...
    return itemRows.filter((row) => {
        const matchedDateRange = matchesDateRange(itemQuery.dateRange, row.inboundDate, row.createdAt.slice(0, 10), itemQuery.timeType === '创建时间');
...
__VLS_asFunctionalElement(__VLS_intrinsicElements.section, __VLS_intrinsicElements.section)({
    ...{ class: "panel item-main-panel stock-transfer-inbound-page" },
});
...
const __VLS_0 = __VLS_asFunctionalComponent(PageTabsLayout, new PageTabsLayout({
    ...
    tabs: (__VLS_ctx.tabs),
    bodyClass: "stock-transfer-inbound-page__body",
}));
...
if (__VLS_ctx.activeTab === 'document') {
    ...
    for (const [option] of __VLS_getVForSourceType((__VLS_ctx.documentTimeTypeOptions))) {
    ...
    var __VLS_45;
    if (!__VLS_ctx.documentFiltersCollapsed) {
        ...
        const __VLS_55 = __VLS_asFunctionalComponent(__VLS_54, new __VLS_54({
            modelValue: (__VLS_ctx.documentQuery.upstreamCode),
    ...
    const __VLS_125 = {
        onClick: (...[$event]) => {
            if (!(__VLS_ctx.activeTab === 'document'))
                return;
            __VLS_ctx.documentFiltersCollapsed = !__VLS_ctx.documentFiltersCollapsed;
        }
    };
    ...
    var __VLS_129;
    (__VLS_ctx.documentFiltersCollapsed ? '展开筛选' : '收起筛选');
    var __VLS_121;
    ...
    /** @type {[typeof CommonQuerySection, typeof CommonQuerySection, ]} */ ;
    // @ts-ignore
    const __VLS_295 = __VLS_asFunctionalComponent(CommonQuerySection, new CommonQuerySection({
    ...
    for (const [option] of __VLS_getVForSourceType((__VLS_ctx.itemTimeTypeOptions))) {
...
/** @type {__VLS_StyleScopedClasses['stock-transfer-inbound-page']} */ ;
...
const __VLS_self = (await import('vue')).defineComponent({
    setup() {
        return {
            ...
            tabs: tabs,
            documentTimeTypeOptions: documentTimeTypeOptions,
            itemTimeTypeOptions: itemTimeTypeOptions,
            ...
            activeTab: activeTab,
            documentFiltersCollapsed: documentFiltersCollapsed,
