...
const menuTree = rawMenu.children;
const buildKey = (indexes) => indexes.map((index) => `m${index}`).join('-');
const buildPath = (segment, indexes) => `/${segment}/${indexes.slice(1).join('/')}`;
...
const buildMenuItems = (nodes, indexes = [], segment = '') => nodes.map((node, index) => {
...
const buildRoutes = (nodes, indexes = [], breadcrumbs = [], segment = '') => nodes.flatMap((node, index) => {
    const currentIndexes = [...indexes, index + 1];
