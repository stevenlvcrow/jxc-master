...
ALTER TABLE IF EXISTS inventory_inventory_check
    ADD COLUMN IF NOT EXISTS stocktake_frequency VARCHAR(16) NOT NULL DEFAULT 'DAILY';
ALTER TABLE IF EXISTS inventory_inventory_check_line
    ADD COLUMN IF NOT EXISTS difference_reason_code VARCHAR(32);
ALTER TABLE IF EXISTS inventory_multi_inventory_check
    ADD COLUMN IF NOT EXISTS stocktake_frequency VARCHAR(16) NOT NULL DEFAULT 'DAILY';
ALTER TABLE IF EXISTS inventory_multi_inventory_check_line
    ADD COLUMN IF NOT EXISTS difference_reason_code VARCHAR(32);
...
FROM (VALUES
    ...
    ('inventory.stocktake_frequency', '盘点频次', 'INVENTORY', 425, '餐饮盘点日盘/周盘频次'),
    ('inventory.check_difference_reason', '盘点差异原因', 'INVENTORY', 426, '餐饮盘点差异原因'),
    ('inventory.profit_loss_result', '盘点盈亏结果', 'INVENTORY', 427, '盘点盈亏结果筛选'),
