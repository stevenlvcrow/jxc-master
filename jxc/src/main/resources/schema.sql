...
CREATE INDEX IF NOT EXISTS idx_sys_dict_type_sort ON sys_dict_type (sort_no, id);

COMMENT ON TABLE sys_dict_type IS '平台全局字典类型表';
COMMENT ON COLUMN sys_dict_type.dict_code IS '字典编码，代码只引用该编码';
COMMENT ON COLUMN sys_dict_type.dict_name IS '字典名称';
COMMENT ON COLUMN sys_dict_type.category IS '字典分类';
COMMENT ON COLUMN sys_dict_type.status IS '状态：ENABLED/DISABLED';
COMMENT ON COLUMN sys_dict_type.builtin IS '是否系统内置';
...
CREATE INDEX IF NOT EXISTS idx_sys_dict_item_type ON sys_dict_item (dict_type_id);
CREATE INDEX IF NOT EXISTS idx_sys_dict_item_parent ON sys_dict_item (parent_id);
CREATE INDEX IF NOT EXISTS idx_sys_dict_item_status ON sys_dict_item (status);
CREATE INDEX IF NOT EXISTS idx_sys_dict_item_sort ON sys_dict_item (sort_no, id);

COMMENT ON TABLE sys_dict_item IS '平台全局字典项表';
COMMENT ON COLUMN sys_dict_item.item_key IS '稳定项键，代码只引用该键';
COMMENT ON COLUMN sys_dict_item.item_code IS '接口和数据库存储值，平台管理员可维护';
COMMENT ON COLUMN sys_dict_item.item_label IS '展示文本';
COMMENT ON COLUMN sys_dict_item.extra_json IS '字典项扩展 JSON';
...
CREATE INDEX IF NOT EXISTS idx_sys_dict_field_binding_code ON sys_dict_field_binding (dict_code);

COMMENT ON TABLE sys_dict_field_binding IS '字典字段绑定表，用于字典存储值变更时迁移历史数据';
...
CREATE TABLE IF NOT EXISTS inventory_batch_balance
(
    id              BIGINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
    scope_type      VARCHAR(16)   NOT NULL,
    scope_id        BIGINT        NOT NULL,
    ...
    created_at      TIMESTAMP     NOT NULL DEFAULT CURRENT_TIMESTAMP,
    updated_at      TIMESTAMP     NOT NULL DEFAULT CURRENT_TIMESTAMP,
    CONSTRAINT uk_inventory_batch_balance_scope_wh_item_batch UNIQUE (scope_type, scope_id, warehouse_name, item_code, batch_no),
    CONSTRAINT ck_inventory_batch_balance_scope_type CHECK (scope_type IN ('PLATFORM', 'GROUP', 'STORE'))
);
CREATE INDEX IF NOT EXISTS idx_inventory_batch_balance_scope ON inventory_batch_balance (scope_type, scope_id, warehouse_name, item_code);
CREATE INDEX IF NOT EXISTS idx_inventory_batch_balance_expiry ON inventory_batch_balance (expiry_date);
COMMENT ON TABLE inventory_batch_balance IS '库存批次结存';

CREATE TABLE IF NOT EXISTS inventory_batch_transaction
(
    id              BIGINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
    scope_type      VARCHAR(16)   NOT NULL,
    scope_id        BIGINT        NOT NULL,
    ...
    operator_id     BIGINT,
    created_at      TIMESTAMP     NOT NULL DEFAULT CURRENT_TIMESTAMP,
    CONSTRAINT ck_inventory_batch_transaction_scope_type CHECK (scope_type IN ('PLATFORM', 'GROUP', 'STORE'))
);
CREATE INDEX IF NOT EXISTS idx_inventory_batch_transaction_scope ON inventory_batch_transaction (scope_type, scope_id, business_date, warehouse_name, item_code);
CREATE INDEX IF NOT EXISTS idx_inventory_batch_transaction_biz ON inventory_batch_transaction (biz_type, biz_id, biz_line_id);
COMMENT ON TABLE inventory_batch_transaction IS '库存批次流水';
...
CREATE TABLE IF NOT EXISTS inventory_inventory_check
(
    id                   BIGINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
    scope_type           VARCHAR(16)   NOT NULL DEFAULT 'PLATFORM',
    scope_id             BIGINT        NOT NULL DEFAULT 0,
    ...
    warehouse_name       VARCHAR(128)  NOT NULL,
    check_range_type     VARCHAR(32)   NOT NULL,
    stocktake_frequency  VARCHAR(16)   NOT NULL DEFAULT 'DAILY',
    freeze_stock         BOOLEAN       NOT NULL DEFAULT FALSE,
    collaborative_flag   BOOLEAN       NOT NULL DEFAULT FALSE,
    plan_name            VARCHAR(128),
    third_party_document  VARCHAR(128),
    salesman_user_id     BIGINT,
    salesman_name        VARCHAR(64),
    item_count           INTEGER       NOT NULL DEFAULT 0,
    total_book_amount    NUMERIC(18,2) NOT NULL DEFAULT 0,
    total_actual_amount  NUMERIC(18,2) NOT NULL DEFAULT 0,
    total_diff_amount    NUMERIC(18,2) NOT NULL DEFAULT 0,
    diff_status          VARCHAR(16)   NOT NULL DEFAULT 'NO_DIFF',
    generated_status     VARCHAR(16),
    print_status         VARCHAR(16)   NOT NULL DEFAULT 'UNPRINTED',
    status               VARCHAR(16)   NOT NULL DEFAULT 'DRAFT',
    remark               VARCHAR(500),
    rejection_reason     VARCHAR(500),
    extra_json           TEXT,
    ...
    created_at           TIMESTAMP     NOT NULL DEFAULT CURRENT_TIMESTAMP,
    updated_at           TIMESTAMP     NOT NULL DEFAULT CURRENT_TIMESTAMP,
    CONSTRAINT uk_inventory_inventory_check_scope_code UNIQUE (scope_type, scope_id, document_code),
    CONSTRAINT ck_inventory_inventory_check_scope_type CHECK (scope_type IN ('PLATFORM', 'GROUP', 'STORE'))
);

CREATE INDEX IF NOT EXISTS idx_inventory_inventory_check_scope ON inventory_inventory_check (scope_type, scope_id, warehouse_name);
CREATE INDEX IF NOT EXISTS idx_inventory_inventory_check_date ON inventory_inventory_check (check_date DESC);

ALTER TABLE inventory_inventory_check
    ADD COLUMN IF NOT EXISTS generated_status VARCHAR(16);
ALTER TABLE inventory_inventory_check
    ADD COLUMN IF NOT EXISTS stocktake_frequency VARCHAR(16) NOT NULL DEFAULT 'DAILY';

COMMENT ON TABLE inventory_inventory_check IS '盘点单';
COMMENT ON COLUMN inventory_inventory_check.scope_type IS '数据范围：PLATFORM/GROUP/STORE';
COMMENT ON COLUMN inventory_inventory_check.scope_id IS '范围ID，平台固定0';
COMMENT ON COLUMN inventory_inventory_check.document_code IS '单据编号';
COMMENT ON COLUMN inventory_inventory_check.check_date IS '盘点日期';
COMMENT ON COLUMN inventory_inventory_check.warehouse_name IS '盘点仓库';
COMMENT ON COLUMN inventory_inventory_check.check_range_type IS '盘点范围类型';
COMMENT ON COLUMN inventory_inventory_check.stocktake_frequency IS '盘点频次：DAILY/WEEKLY';
COMMENT ON COLUMN inventory_inventory_check.freeze_stock IS '是否冻结库存';
COMMENT ON COLUMN inventory_inventory_check.collaborative_flag IS '是否多人协同盘点';
COMMENT ON COLUMN inventory_inventory_check.plan_name IS '盘点方案名称';
COMMENT ON COLUMN inventory_inventory_check.third_party_document IS '第三方单号';
COMMENT ON COLUMN inventory_inventory_check.salesman_user_id IS '盘点人用户ID';
COMMENT ON COLUMN inventory_inventory_check.salesman_name IS '盘点人名称';
COMMENT ON COLUMN inventory_inventory_check.item_count IS '盘点物品数';
COMMENT ON COLUMN inventory_inventory_check.total_book_amount IS '账面金额';
COMMENT ON COLUMN inventory_inventory_check.total_actual_amount IS '实盘金额';
COMMENT ON COLUMN inventory_inventory_check.total_diff_amount IS '差异金额';
COMMENT ON COLUMN inventory_inventory_check.diff_status IS '差异状态：NO_DIFF/PROFIT/LOSS';
COMMENT ON COLUMN inventory_inventory_check.generated_status IS '生成状态，普通盘点为空，多人盘点使用独立表';
COMMENT ON COLUMN inventory_inventory_check.print_status IS '打印状态：UNPRINTED/PRINTED';
COMMENT ON COLUMN inventory_inventory_check.status IS '状态：DRAFT/SUBMITTED/APPROVED';
COMMENT ON COLUMN inventory_inventory_check.rejection_reason IS '拒审原因';
COMMENT ON COLUMN inventory_inventory_check.extra_json IS '扩展字段';

CREATE TABLE IF NOT EXISTS inventory_inventory_check_line
(
    id                   BIGINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
    header_id            BIGINT        NOT NULL,
    item_code            VARCHAR(64)   NOT NULL,
    ...
    book_price           NUMERIC(18,4) NOT NULL DEFAULT 0,
    ...
    profit_qty           NUMERIC(18,4) NOT NULL DEFAULT 0,
    loss_qty             NUMERIC(18,4) NOT NULL DEFAULT 0,
    profit_loss_reason   VARCHAR(256),
    difference_reason_code VARCHAR(32),
    profit_inbound_price NUMERIC(18,4) NOT NULL DEFAULT 0,
    profit_amount        NUMERIC(18,2) NOT NULL DEFAULT 0,
    loss_outbound_price  NUMERIC(18,4) NOT NULL DEFAULT 0,
    loss_amount          NUMERIC(18,2) NOT NULL DEFAULT 0,
    abnormal_flag        VARCHAR(16)   NOT NULL DEFAULT 'NORMAL',
    remark               VARCHAR(500),
    extra_json           TEXT,
    created_at           TIMESTAMP     NOT NULL DEFAULT CURRENT_TIMESTAMP
);

CREATE INDEX IF NOT EXISTS idx_inventory_inventory_check_line_header ON inventory_inventory_check_line (header_id, id);
ALTER TABLE inventory_inventory_check_line
    ADD COLUMN IF NOT EXISTS difference_reason_code VARCHAR(32);

COMMENT ON TABLE inventory_inventory_check_line IS '盘点单行';
COMMENT ON COLUMN inventory_inventory_check_line.header_id IS '单据ID';
COMMENT ON COLUMN inventory_inventory_check_line.available_qty IS '账面可用数量';
COMMENT ON COLUMN inventory_inventory_check_line.book_qty IS '账面数量';
COMMENT ON COLUMN inventory_inventory_check_line.actual_qty IS '实盘数量';
COMMENT ON COLUMN inventory_inventory_check_line.diff_qty IS '差异数量';
COMMENT ON COLUMN inventory_inventory_check_line.profit_qty IS '盘盈数量';
COMMENT ON COLUMN inventory_inventory_check_line.loss_qty IS '盘亏数量';
COMMENT ON COLUMN inventory_inventory_check_line.profit_loss_reason IS '盘盈盘亏原因';
COMMENT ON COLUMN inventory_inventory_check_line.difference_reason_code IS '差异原因编码';
COMMENT ON COLUMN inventory_inventory_check_line.abnormal_flag IS '异常标记：NORMAL/ABNORMAL';

CREATE TABLE IF NOT EXISTS inventory_multi_inventory_check
(
    id                   BIGINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
    scope_type           VARCHAR(16)   NOT NULL DEFAULT 'PLATFORM',
    scope_id             BIGINT        NOT NULL DEFAULT 0,
    ...
    stocktake_frequency  VARCHAR(16)   NOT NULL DEFAULT 'DAILY',
    freeze_stock         BOOLEAN       NOT NULL DEFAULT FALSE,
    collaborative_flag   BOOLEAN       NOT NULL DEFAULT TRUE,
    generated_status     VARCHAR(16)   NOT NULL DEFAULT 'UNGENERATED',
    print_status         VARCHAR(16)   NOT NULL DEFAULT 'UNPRINTED',
    status               VARCHAR(16)   NOT NULL DEFAULT 'DRAFT',
    remark               VARCHAR(500),
    rejection_reason     VARCHAR(500),
    ...
    created_at           TIMESTAMP     NOT NULL DEFAULT CURRENT_TIMESTAMP,
    updated_at           TIMESTAMP     NOT NULL DEFAULT CURRENT_TIMESTAMP,
    CONSTRAINT uk_inventory_multi_inventory_check_scope_code UNIQUE (scope_type, scope_id, document_code),
    CONSTRAINT ck_inventory_multi_inventory_check_scope_type CHECK (scope_type IN ('PLATFORM', 'GROUP', 'STORE'))
);

CREATE INDEX IF NOT EXISTS idx_inventory_multi_inventory_check_scope ON inventory_multi_inventory_check (scope_type, scope_id, warehouse_name);
CREATE INDEX IF NOT EXISTS idx_inventory_multi_inventory_check_date ON inventory_multi_inventory_check (check_date DESC);

COMMENT ON TABLE inventory_multi_inventory_check IS '多人盘点单';
ALTER TABLE inventory_multi_inventory_check
    ADD COLUMN IF NOT EXISTS stocktake_frequency VARCHAR(16) NOT NULL DEFAULT 'DAILY';
COMMENT ON COLUMN inventory_multi_inventory_check.generated_status IS '生成状态：UNGENERATED/GENERATED';

CREATE TABLE IF NOT EXISTS inventory_multi_inventory_check_line
(
    id                   BIGINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
    header_id            BIGINT        NOT NULL,
    item_code            VARCHAR(64)   NOT NULL,
...
CREATE INDEX IF NOT EXISTS idx_inventory_multi_inventory_check_line_header ON inventory_multi_inventory_check_line (header_id, id);
ALTER TABLE inventory_multi_inventory_check_line
    ADD COLUMN IF NOT EXISTS difference_reason_code VARCHAR(32);

COMMENT ON TABLE inventory_multi_inventory_check_line IS '多人盘点单行';
...
(
    ...
    created_at     TIMESTAMP     NOT NULL DEFAULT CURRENT_TIMESTAMP,
    updated_at     TIMESTAMP     NOT NULL DEFAULT CURRENT_TIMESTAMP,
    CONSTRAINT ck_inventory_stock_lock_scope_type CHECK (scope_type IN ('PLATFORM', 'GROUP', 'STORE'))
);
CREATE INDEX IF NOT EXISTS idx_inventory_stock_lock_active ON inventory_stock_lock (scope_type, scope_id, warehouse_name, item_code, active);
CREATE INDEX IF NOT EXISTS idx_inventory_stock_lock_source ON inventory_stock_lock (source_type, source_id, active);
COMMENT ON TABLE inventory_stock_lock IS '库存锁库记录';
...
-- 业务状态和业务类型统一改由平台字典校验，保留作用域、审计、登录、导入等系统轨迹约束。
DELETE FROM inventory_inventory_check_line
WHERE header_id IN (
    SELECT id FROM inventory_inventory_check
    WHERE stocktake_frequency IN ('MONTHLY', '月盘点')
);
DELETE FROM inventory_inventory_check
WHERE stocktake_frequency IN ('MONTHLY', '月盘点');
UPDATE inventory_inventory_check SET stocktake_frequency = 'DAILY' WHERE stocktake_frequency = '日盘点';
UPDATE inventory_inventory_check SET stocktake_frequency = 'WEEKLY' WHERE stocktake_frequency = '周盘点';
DELETE FROM inventory_inventory_check_line
WHERE header_id IN (
    SELECT id FROM inventory_inventory_check
    WHERE COALESCE(stocktake_frequency, '') NOT IN ('DAILY', 'WEEKLY')
);
DELETE FROM inventory_inventory_check
WHERE COALESCE(stocktake_frequency, '') NOT IN ('DAILY', 'WEEKLY');
DELETE FROM inventory_multi_inventory_check_line
WHERE header_id IN (
    SELECT id FROM inventory_multi_inventory_check
    WHERE stocktake_frequency IN ('MONTHLY', '月盘点')
);
DELETE FROM inventory_multi_inventory_check
WHERE stocktake_frequency IN ('MONTHLY', '月盘点');
UPDATE inventory_multi_inventory_check SET stocktake_frequency = 'DAILY' WHERE stocktake_frequency = '日盘点';
UPDATE inventory_multi_inventory_check SET stocktake_frequency = 'WEEKLY' WHERE stocktake_frequency = '周盘点';
DELETE FROM inventory_multi_inventory_check_line
WHERE header_id IN (
    SELECT id FROM inventory_multi_inventory_check
    WHERE COALESCE(stocktake_frequency, '') NOT IN ('DAILY', 'WEEKLY')
...
UPDATE inventory_inventory_check_line
SET difference_reason_code = CASE profit_loss_reason
    WHEN '自然损耗' THEN 'NATURAL_LOSS'
    WHEN '加工损耗' THEN 'PROCESSING_LOSS'
    WHEN '过期变质' THEN 'EXPIRED_SPOILED'
    WHEN '领用未开单' THEN 'UNBILLED_PICKING'
    WHEN '称重不准' THEN 'WEIGHING_ERROR'
    WHEN '单位换算错' THEN 'UNIT_CONVERSION_ERROR'
    ELSE difference_reason_code
END
WHERE COALESCE(diff_qty, 0) <> 0
  AND difference_reason_code IS NULL
  AND profit_loss_reason IN ('自然损耗', '加工损耗', '过期变质', '领用未开单', '称重不准', '单位换算错');
UPDATE inventory_multi_inventory_check_line
SET difference_reason_code = CASE profit_loss_reason
    WHEN '自然损耗' THEN 'NATURAL_LOSS'
    WHEN '加工损耗' THEN 'PROCESSING_LOSS'
    WHEN '过期变质' THEN 'EXPIRED_SPOILED'
...
WITH invalid_headers AS (
    ...
    WHERE COALESCE(diff_qty, 0) <> 0
      AND COALESCE(difference_reason_code, '') NOT IN (
          'NATURAL_LOSS', 'PROCESSING_LOSS', 'EXPIRED_SPOILED',
          'UNBILLED_PICKING', 'WEIGHING_ERROR', 'UNIT_CONVERSION_ERROR'
      )
),
deleted_lines AS (
    DELETE FROM inventory_inventory_check_line
    WHERE header_id IN (SELECT header_id FROM invalid_headers)
    RETURNING header_id
)
DELETE FROM inventory_inventory_check
WHERE id IN (SELECT header_id FROM deleted_lines);
WITH invalid_headers AS (
    SELECT DISTINCT header_id
...
UPDATE item_profile
SET detail_json = (detail_json::jsonb - 'stocktakeFrequency' - 'stocktakeTypes')::text
WHERE detail_json IS NOT NULL
  AND (
      detail_json::jsonb ->> 'stocktakeFrequency' IN ('MONTHLY', '月盘点')
      OR (
          detail_json::jsonb ? 'stocktakeTypes'
          AND (
              detail_json::jsonb -> 'stocktakeTypes' ? 'MONTHLY'
              OR detail_json::jsonb -> 'stocktakeTypes' ? '月盘点'
          )
      )
  );
UPDATE item_profile
SET detail_json = jsonb_set(
        detail_json::jsonb - 'stocktakeTypes',
        '{stocktakeFrequency}',
        CASE detail_json::jsonb ->> 'stocktakeFrequency'
            WHEN '日盘点' THEN to_jsonb('DAILY'::text)
            WHEN '周盘点' THEN to_jsonb('WEEKLY'::text)
            ELSE to_jsonb(detail_json::jsonb ->> 'stocktakeFrequency')
        END,
        TRUE
    )::text
WHERE detail_json IS NOT NULL
  AND detail_json::jsonb ? 'stocktakeFrequency'
  AND detail_json::jsonb ->> 'stocktakeFrequency' IN ('日盘点', '周盘点', 'DAILY', 'WEEKLY');
UPDATE item_profile
SET detail_json = (
        CASE
            WHEN detail_json::jsonb ? 'stocktakeTypes'
                 AND jsonb_array_length(detail_json::jsonb -> 'stocktakeTypes') > 0
                 AND (detail_json::jsonb -> 'stocktakeTypes' ->> 0) IN ('日盘点', '周盘点', 'DAILY', 'WEEKLY')
                THEN jsonb_set(
                    detail_json::jsonb - 'stocktakeTypes',
                    '{stocktakeFrequency}',
                    CASE detail_json::jsonb -> 'stocktakeTypes' ->> 0
                        WHEN '日盘点' THEN to_jsonb('DAILY'::text)
                        WHEN '周盘点' THEN to_jsonb('WEEKLY'::text)
                        ELSE to_jsonb(detail_json::jsonb -> 'stocktakeTypes' ->> 0)
                    END,
                    TRUE
                )
            ELSE detail_json::jsonb - 'stocktakeTypes'
        END
    )::text
WHERE detail_json IS NOT NULL
  AND detail_json::jsonb ? 'stocktakeTypes';
...
ALTER TABLE sys_user_role_rel DROP CONSTRAINT IF EXISTS ck_user_role_status;
ALTER TABLE sys_store_admin_rel DROP CONSTRAINT IF EXISTS ck_store_admin_status;
ALTER TABLE item_category DROP CONSTRAINT IF EXISTS ck_item_category_status;
ALTER TABLE item_tag DROP CONSTRAINT IF EXISTS ck_item_tag_status;
ALTER TABLE dish_category DROP CONSTRAINT IF EXISTS ck_dish_category_status;
...
DROP TABLE IF EXISTS inventory_profit_inbound_line;
DROP TABLE IF EXISTS inventory_profit_inbound;
DROP TABLE IF EXISTS inventory_loss_outbound_line;
DROP TABLE IF EXISTS inventory_loss_outbound;
